...
def generate_launch_description():
    teleop_share_dir = get_package_share_directory('wave_rover_logitech_f710_teleop')
    control_share_dir = get_package_share_directory('wave_rover_control')

    joy_dev = LaunchConfiguration('joy_dev')
    params_file = LaunchConfiguration('params_file')
    f710_config = os.path.join(teleop_share_dir, 'config', 'f710_teleop.yaml')
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description='Joystick device for the Logitech F710 controller'
        ),
        DeclareLaunchArgument(
            'params_file',
